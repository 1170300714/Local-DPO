...
_MP: Optional[GroupCoordinator] = None
...
# MP: Model Parallel (orthogonal to DP)
def get_mp_group() -> GroupCoordinator:
    assert _MP is not None, "model parallel group is not initialized"
    return _MP
...
def initialize_model_parallel(
    ...
    global _MP
    assert _MP is None, "model parallel group is already initialized"
    _MP = init_model_parallel_group(
        group_ranks=[list(rs) for rs in zip(*rank_generator.get_ranks("dp"))],
        local_rank=get_world_group().local_rank,
        backend=backend,
        parallel_mode="model",
    )
