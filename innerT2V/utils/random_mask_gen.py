...
def create_random_shape_with_random_motion_multiple_connected_components(video_length, zoomin, zoomout, rotmin, rotmax, cc_ratio, fix_area, imageHeight=240, imageWidth=432):
    # get a random shape
    assert zoomin < 1, "Zoom-in parameter must be smaller than 1"
    assert zoomout > 1, "Zoom-out parameter must be larger than 1"
    assert rotmin < rotmax, "Minimum value of rotation must be smaller than maximun value !"
    ...
    region_width, region_height = region.size
    if fix_area == 1:
        region = region.resize((int(region_width // cc_ratio), int(region_height // cc_ratio)), Image.Resampling.NEAREST)
        region_width, region_height = region.size
    else:
        resize_mask = random.randint(0, 1)
        if resize_mask == 1:
            region = region.resize((int(region_width // cc_ratio), int(region_height // cc_ratio)), Image.Resampling.NEAREST)
            region_width, region_height = region.size
    # get random position
...
if __name__ == '__main__':
    ...
    for _ in range(trials):
        ...
        masks = create_random_shape_with_random_motion(video_length, zoomin=0.9, zoomout=1.1, rotmin=1, rotmax=10, imageHeight=480, imageWidth=720)
        i = 0
    print(time.time()-start)
        ...
        #     imageio.imwrite('mask_vis/{:05d}.png'.format(i), cur_mask)
        #     i += 1
