...
def save_tensor_as_video_imageio(tensor, filename, fps=30):
    video = tensor.squeeze(0)  # (T, C, H, W)
    if video.dtype == torch.float32:
        video = (video.clamp(0, 1) * 255).byte()
    video = video.permute(0, 2, 3, 1)  # (T, H, W, C)
    video_np = video.cpu().numpy()
    imageio.mimwrite(filename, video_np, fps=fps, macro_block_size=None)
...
def main(cml_args=None, only_get_args: bool = False):
    ...
    vae = AutoencoderKLWan.from_pretrained(args.base_modules_dir, subfolder="vae", torch_dtype=torch.float32)
    pipe = WanLocalDPOPipeline.from_pretrained(args.base_modules_dir, vae=vae, torch_dtype=torch.bfloat16)
